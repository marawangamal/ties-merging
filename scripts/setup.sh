...
eval_split=validation; \
method=ta; \
sweep_str="linear+0.1+1.01+0.1"; \
model_name=t5_large; \
python ./src/ties_merging.py -c configs/${model_name}.json -i ${datasets_to_inference} -m ${datasets_to_merge} -f opm::${method}::${sweep_str} --kwargs split=${eval_split} project_name=evaluation experiment_name=opm_${method}
...
# 6. Evaluate on test set (no sweep)
datasets_to_merge=qasc,wiki_qa,quartz,paws,winogrande,wsc; \
datasets_to_inference=qasc,wiki_qa,quartz,paws,winogrande,wsc; \
eval_split=test; \
method=isoc; \
...
--log_to_csv=results/results_sweep.csv \
