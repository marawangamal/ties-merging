...
def mix_avg_default(pt_tensor, task_tensors, key, scaling_coefficient=1.0, **kwargs):
...
def mix_add_default(pt_tensor, task_tensors, key, scaling_coefficient=1.0, **kwargs):
    if "wo" in key:  # TA
        print(f"[{key}] Using WA")
        return pt_tensor + task_tensors.mean(dim=0)
    else:
        return pt_tensor + task_tensors.sum(dim=0) * scaling_coefficient
...
def merge_dummy(pt_tensor, task_tensors, key, **kwargs):
    return pt_tensor
...
def opmerge(
    ...
    **merge_kwargs,
    ...
    merge_fn = {
        ...
        "wa": merge_wa,
        "ta": merge_ta,
        "mix_avg_default": mix_avg_default,
        "mix_add_default": mix_add_default,
        "tsv": merge_tsv,
        "isoc": merge_isoc,
